...
class Configspace:  # shows the way of the robot the algorithm
    ...
    def __init__(self, robotImagePath, root):
        ...
        self.theOffsetX = Image.open(robotImagePath).width  # haf of the pixel of the robot png.
        self.theOffsetY = Image.open(robotImagePath).height

    def setDimensions(self, x, y):  # of the canvas
        self.xExt = x  # set Max x
        self.yExt = y  # set Max Y
        self.canvas.config(bd=0, height=y + 2 * self.theOffsetY, width=x + 2 * self.theOffsetX)  # bd = Border just
    ...
    def drawSpace(self):
        ...
        self.canvas.create_line(self.theOffsetX, self.theOffsetY, self.theOffsetX, y + self.theOffsetY)  # left border
        self.canvas.create_line(self.theOffsetX, self.theOffsetY, x + self.theOffsetX, self.theOffsetY)  # upper border
        self.canvas.create_line(x + self.theOffsetX, y + self.theOffsetY, x + self.theOffsetX, self.theOffsetY)  # right
        self.canvas.create_line(x + self.theOffsetX, y + self.theOffsetY, self.theOffsetX, y + self.theOffsetY)  # lower
    ...
    def drawConfiguration(self, x, y, color):  # draws a dot on the Canvas (used above for the start and goal)
        ...
        self.canvas.create_oval(x - r + self.theOffsetX,
                                y - r + self.theOffsetY,
                                x + r + self.theOffsetX,
                                y + r + self.theOffsetY,
    ...
    def drawSolutionPath(self):  # Draws a line connecting all the points from the solution-path
        for i in range(1, len(self.solutionPath)):  # iterate over points from solution-path
            ...
            self.canvas.create_line(c1[0] + self.theOffsetX, c1[1] + self.theOffsetY,
                                    c2[0] + self.theOffsetX, c2[1] + self.theOffsetY, fill='purple1')
